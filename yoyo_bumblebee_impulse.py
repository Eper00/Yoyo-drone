...
class YoyoBumblebee1DOF_impulse(bumblebee.Bumblebee):
    """
    ...
    theta =105.6 #theta_max at the initial position
    theta_dot = 0
    theta_ddot = 0
    d = 0
    r = 0
    ...
    def get_identifiers(cls) -> Optional[list[str]]:
        return ["YoyoBumblebee1DOF_impulse"]
    ...
    def create_xml_element(self, pos: str, quat: str, color: str) -> dict[str, list[ET.Element]]:
        ...
        yoyo = ET.SubElement(drone_body, "body", name=yoyo_name, pos="0 0 0")
    ...
    def update(self) -> None:
        ...
        self.r_theta = YoyoBumblebee1DOF_impulse.r_0 + (self.theta * YoyoBumblebee1DOF_impulse.alpha)
       
        # Yoyo control(based on are we in the pusle time)
        # --- Yoyo control ---
        if self.z < self.z_set - 0.01 and not self.pulse_active:
            self.pulse_active = True

            self.timer=0.001+self.timer
            self.h_ddot = self.h_ddot_r
            if self.timer>self.P:
                self.timer=0
                self.pulse_active=False       
            
        else:
                
            self.h_ddot=-2.3*cur_vel[2]-4.2*(cur_pos[2]-1)
            if self.z_max>self.z_set and self.z_min<self.z_set:
                

                arg1 = 4*(self.z_max-self.z_set)/(9.81*(1-self.beta))
                arg2 = 4*(self.z_set-self.z_min)/(9.81*(1-self.beta))
                if arg1 > 0 and arg2 > 0:
                    t1 = np.sqrt(arg1)
                    self.P = np.sqrt(t1*t1 + arg2) - t1
            if self.z<self.z_set:
                self.pulse_active=True
        self.h += self.h_dot * self.dt + 0.5 * self.h_ddot * self.dt**2
        self.h_dot += self.h_ddot * self.dt
        # Yoyo dynamics
        # Impact
        if self.theta <= 0 and self.theta_dot < 0:
            self.z_min=self.z
            self.z_maxpast=self.z_max

            self.z_max=0
            self.T_imp = round((2 * 3.14)/(abs(self.theta_dot) * (1 + YoyoBumblebee1DOF_impulse.beta)) / self.dt,0)
            self.theta_dot = - YoyoBumblebee1DOF_impulse.beta * self.theta_dot
            self.yoyo_z = cur_pos[2] - 2.2 - YoyoBumblebee1DOF_impulse.L
            self.T = YoyoBumblebee1DOF_impulse.m_yoyo * ((((1 + YoyoBumblebee1DOF_impulse.beta) * abs(self.theta_dot) * YoyoBumblebee1DOF_impulse.r_0) / (self.dt * self.T_imp)) + cur_acc[2] - 9.81)
        else:
            
            if self.z>self.z_max:
                self.z_max=self.z
            self.T = YoyoBumblebee1DOF_impulse.m_yoyo * (cur_acc[2] + (YoyoBumblebee1DOF_impulse.alpha * self.theta_dot * self.theta_dot) * (self.r_theta * self.theta_ddot))
            self.theta_ddot = - (self.T * self.r_theta / YoyoBumblebee1DOF_impulse.J_yoyo)
        ...
        self.z = cur_pos[2] - YoyoBumblebee1DOF_impulse.L + (YoyoBumblebee1DOF_impulse.r_0 * self.theta) + ((YoyoBumblebee1DOF_impulse.alpha / 2) * (self.theta * self.theta))
        self.z_dot = cur_vel[2] + (self.r_theta * self.theta_dot)
        self.z_ddot = (cur_acc[2] - 9.81) + (YoyoBumblebee1DOF_impulse.alpha * self.theta_dot * self.theta_dot) + (self.r_theta * self.theta_ddot)

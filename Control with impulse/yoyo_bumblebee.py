...
class YoyoBumblebee1DOF(bumblebee.Bumblebee):
    """
    ...
    def update(self) -> None:
        ...
        self.r_theta = YoyoBumblebee1DOF.r_0 + (self.theta * YoyoBumblebee1DOF.alpha)
       

        
        # Yoyo control(based on are we in the pusle time)
        # --- Yoyo control ---
        if self.pulse_active==True:
            ...
            if self.z_max>self.z_set and self.z_min<self.z_set:
                ...
                t1=np.sqrt(4*(self.z_max-self.z_set)/(9.81*(1-self.beta))) 
                self.P=self.K*np.sqrt(t1*t1+4*(self.z_set-self.z_min)/(9.81*(1-self.beta)))-t1
            self.h = 1
            self.h_dot = 0.0
            self.h_ddot = 0.0
        ...
        if self.theta <= 0 and self.theta_dot < 0:
            ...
            self.z_max=0
            self.T_imp = round((2 * 3.14)/(abs(self.theta_dot) * (1 + YoyoBumblebee1DOF.beta)) / self.dt,0)
            self.theta_dot = - YoyoBumblebee1DOF.beta * self.theta_dot
            self.yoyo_z = cur_pos[2] - 2.2 - YoyoBumblebee1DOF.L
            self.T = YoyoBumblebee1DOF.m_yoyo * ((((1 + YoyoBumblebee1DOF.beta) * abs(self.theta_dot) * YoyoBumblebee1DOF.r_0) / (self.dt * self.T_imp)) + cur_acc[2] - 9.81)
        else:
            
            if self.z>self.z_max:
                self.z_max=self.z
            self.T = YoyoBumblebee1DOF.m_yoyo * (cur_acc[2] + (YoyoBumblebee1DOF.alpha * self.theta_dot * self.theta_dot) * (self.r_theta * self.theta_ddot))
            self.theta_ddot = - (self.T * self.r_theta / YoyoBumblebee1DOF.J_yoyo)
        ...
        self.z = cur_pos[2] - YoyoBumblebee1DOF.L + (YoyoBumblebee1DOF.r_0 * self.theta) + ((YoyoBumblebee1DOF.alpha / 2) * (self.theta * self.theta))
        self.z_dot = cur_vel[2] + (self.r_theta * self.theta_dot)
        self.z_ddot = (cur_acc[2] - 9.81) + (YoyoBumblebee1DOF.alpha * self.theta_dot * self.theta_dot) + (self.r_theta * self.theta_ddot)
        ...
        if self.index % 500 == 0 and self.index/500 <= 30 and self.index != 0:

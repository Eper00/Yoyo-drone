...
from aiml_virtual.simulated_object.dynamic_object.controlled_object.drone import yoyo_bumblebee_MPC

if __name__ == "__main__":
    
    scn = scene.Scene(os.path.join(xml_directory, "empty_checkerboard.xml"), save_filename=f"result.xml")

    bby = yoyo_bumblebee_MPC.YoyoBumblebee1DOF_MPC()
    scn.add_object(bby, "0 0 1", "1 0 0 0", "0.5 0.5 0.5 1")
    bby.update_frequency=int(1/0.001)
    sim = simulator.Simulator(scn)
    
    with sim.launch(fps=100,with_display=True):

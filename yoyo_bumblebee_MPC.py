...
from scripts.my_code.MPC_Casadi.control import Controller
from scripts.my_code.utils import parameters,support
...
class YoyoBumblebee1DOF_MPC(bumblebee.Bumblebee):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #---- Vezérlés
        self.control=Controller(parameters.x0_val, parameters.x0_init, parameters.N, 
                                parameters.M, parameters.params,support.multiple_rk4_steps)
    @classmethod
    ...
    def update(self) -> None:
        ...
        if self.time < self.max_computing_time and (len(self.h_opt_interp_before)!=0) and False:
    ...
    def update_control(self):
        self.index_control=0
        if (self.theta<0):
            self.control.x0_val[0] = -self.theta
        else:
            self.control.x0_val[0] = self.theta
        self.control.x0_val[1] = self.theta_dot
        self.control.x0_val[2] = self.state['pos'][2]
        self.control.x0_val[3] = self.state['vel'][2]
        ...
        print(self.control.x0_val)
        w_opt=self.control.solve()
        self.control.x0_val[4] = w_opt[0]
        self.control.x0_init = w_opt
